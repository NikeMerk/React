...
export const PostView: FC<IPostProps> = ({
  ...
  const changePostQuery = useMutation(
    {
      mutationFn: () => changePost(!post.done, post.id),
      onSuccess() {
  ...
  const clickButtonDone = (): void => {
    if (stateStorApp) {
      changePostQuery.mutate();
    } else {
      localArray.map((obj) => {
        if (obj.id === post.id) {
          obj.done = !obj.done;
  ...
  return (
    ...
    >
      ...
      <div className="list__item-blok-buttons">
        <button className="list__item-btn-done" onClick={clickButtonDone}>
          <svg
            width="17.188263"
            height="12.375916"
        ...
        <button className="list__item-btn-delete" onClick={clickButtonDelete}>
          <svg
            width="16.500000"
            height="17.875000"
